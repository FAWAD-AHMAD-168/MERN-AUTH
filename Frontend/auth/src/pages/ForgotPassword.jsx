...
import { LucideClockFading, Mail } from "lucide-react";
...
try {
    ...
    reset();
    navigate("/forgot-password/reset-otp")
  ...
  // ✅ Check if user is on the main "/forgot-password" route
  const isMainRoute = location.pathname === "/forgot-password";
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-r from-blue-500 via-blue-400 to-indigo-500">
      <div className="w-full max-w-md bg-white/95 backdrop-blur-md shadow-2xl rounded-2xl p-8 border border-blue-100">
        {isMainRoute ? (
          ...
          // ✅ Render ResetOTP or NewPassword inside this card
          <Outlet />
        )}
      </div>
    </div>
